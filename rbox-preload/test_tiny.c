...
extern const unsigned char tiny_data[];
extern const size_t tiny_size;
...
int tiny_init() {
    if (tiny_size < sizeof(dfa_header_t)) return 0;
    g_header = (dfa_header_t*)tiny_data;
    if (g_header->magic != DFA_MAGIC) return 0;
    
    g_states = (dfa_state_t*)(tiny_data + g_header->initial_state);
    g_alpha_map = (uint8_t*)(tiny_data + sizeof(dfa_header_t));
    ...
    printf("Alphabet: 'g'->%d, 'l'->%d, 's'->%d\n",
           g_alpha_map[0x67], g_alpha_map[0x6c], g_alpha_map[0x73]);
...
int evaluate(const char* input) {
    ...
    for (size_t pos = 0; pos < len; pos++) {
        ...
        for (int i = 0; i < state->transition_count; i++) {
            uint8_t ch = tiny_data[g_trans_start + state->transitions_offset + i * 5];
            uint32_t next = *(uint32_t*)&tiny_data[g_trans_start + state->transitions_offset + i * 5 + 1];
            printf("    trans[%d]: char=%d, next=%d\n", i, ch, next);
            if (ch == symbol) {
                state = (dfa_state_t*)(tiny_data + next);
                found = 1;
                break;
            }
        }
...
int main() {
    ...
    const char* tests[] = {"gl", "ls", "g", "l", "s", "gg"};
    for (int i = 0; i < 6; i++) {
