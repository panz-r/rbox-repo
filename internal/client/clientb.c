...
/* Configuration /
static char g_socket_path1024 = "/tmp/readonlybox.sock";
static int g_socket_fd = -1;
static int g_request_in_progress = 0;  / Flag: request currently in progress /
...
/* Generate UUID /
static void generate_uuid(unsigned char uuid16) {
...
static void next_request_uuid(unsigned char uuid16) {
    / Add 1 to UUID as a 128-bit integer */
    int carry = 1;
...
/* Queue a message for async sending - with safe lazy init */
static void queue_message(const char msg) {
    / Use atomic flag to prevent multiple lazy_init calls */
    static volatile int init_in_progress = 0;
    static int init_done = 0;
    
    if (!init_done && __sync_fetch_and_add(&init_in_progress, 1) == 1) {
        lazy_init();
        init_done = 1;
    }
...
static void keepalive_thread(void arg) {
    while (g_keepalive_running) {
        ...
        if (idle_time > g_keepalive_timeout && g_socket_fd >= 0 && !g_request_in_progress) {
            pthread_mutex_lock(&g_socket_mutex);
            if (g_socket_fd >= 0 && !g_request_in_progress) {
                close(g_socket_fd);
                g_socket_fd = -1;
            }
            pthread_mutex_unlock(&g_socket_mutex);
...
static int check_decision_cache(const char *cmd, uint8_t out_decision, char out_reason, size_t reason_size) {
    ...
    /* Opportunistically clean expired entries from end */
...
static int has_write_operation(char *const argv[]) {
    for (int i = 0; argv && argvi; i++) {
        if (strcmp(argvi, ">") == 0 || strcmp(argvi, ">>") == 0 ||
            strcmp(argvi, "2>") == 0 || strcmp(argvi, "&>") == 0 ||
            strcmp(argvi, "1>") == 0 || strcmp(argvi, "2>>") == 0) {
            return 1;
        }
    }
    return 0;
...
static int send_request(const char *cmd, char const argv[], char const envp[],
    ...
    int argc = 0, envc = 0;
    if (argv) { while (argvargc) argc++; }
    if (envp) { while (envpenvc) envc++; }
    /* Build packet */
    char packet[8192];
    size_t pos = 0;
...
static void send_log(const char *format, ...) {
    ...
    uint32_t version = ROBO_VERSION;
    uint32_t argc = 0;
    uint32_t envc = 1;  /* We'll put log message as a fake env var */
    memcpy(packet + pos, &magic, 4);
    pos += 4;
    memcpy(packet + pos, &version, 4);
    pos += 4;
...
static int check_and_execute(const char *syscall_name, const char path, char const argv[], char const envp[]) {
    ...
    if (result < 0) {
        /* Denied - no logging to server to avoid deadlock */
        errno = EACCES;
        return -1;
    }
    /* Allowed - log to server */
...
int execve(const char *path, char const argv[], char const envp[]) {
    ...
    if (check < 0) {
        /* Denied - set errno and return error */
        errno = EACCES;
        return -1;
    }
...
int posix_spawnp(pid_t *pid, const char file,
    ...
    /* For spawnp, we need to resolve path first - this is complex
