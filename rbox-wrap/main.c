...
static int init_embedded_dfa(void) {
    ...
    g_dfa_data = readonlybox_dfa_data;
    g_dfa_size = readonlybox_dfa_data_size;
    
    /* Validate identifier */
    if (!dfa_eval_validate_id(g_dfa_data, g_dfa_size, EXPECTED_IDENTIFIER)) {
        if (!dfa_eval_validate_id(g_dfa_data, g_dfa_size, "ReadOnlyBox")) {
            g_dfa_data = NULL;
            g_dfa_size = 0;
            return -1;
        }
...
int main(int argc, char *argv[]) {
    ...
    int opt;
    int option_index = 0;
    while ((opt = getopt_long(argc, argv, "s:u:hrbcblv", 
