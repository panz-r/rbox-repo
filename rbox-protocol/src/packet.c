...
rbox_error_t rbox_build_request(char *packet, size_t capacity, size_t *out_len,
    ...
    /* Set caller and syscall in header (v9) */
...
rbox_session_state_t rbox_session_heartbeat(rbox_session_t *session, short events) {
    ...
    switch (session->state) {
        ...
        case RBOX_SESSION_WAITING:
            if (events & POLLIN) {
                char buf[4096];  /* Larger buffer for response */
                short events;
                int fd = rbox_session_pollfd(session, &events);
                ...
                if (n > 0) {
                    ...
                    /* Validate response using accumulated data, not just the read buffer */
                    if (session->response_len >= RBOX_HEADER_SIZE) {
                        rbox_response_t resp;
                        session->error = validate_response(session->response_data, session->response_len, session->request_id, &resp);
                        if (session->error == RBOX_OK) {
                            session->response = resp;
                            session->state = RBOX_SESSION_RESPONSE_READY;
                        } else if (session->error == RBOX_ERR_TRUNCATED) {
                            /* Partial response - need more data, continue waiting */
                            /* Don't change state, keep waiting */
