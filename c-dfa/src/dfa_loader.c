...
const char* lookup_capture_name(const void* dfa_data, uint16_t uid, char* out_buf, size_t buf_size) {
    if (!dfa_data || uid == 0 || !out_buf || buf_size == 0) return NULL;
    ...
    // Each entry has at minimum UID(4) + name_len(2) = 6 bytes
    // Cap entry_count to prevent billions of iterations with corrupted data
    size_t max_metadata_size = 32768;
    if (entry_count > max_metadata_size / 6) entry_count = max_metadata_size / 6;
    ...
    for (uint32_t i = 0; i < entry_count && byte_offset + 6 <= max_metadata_size; i++) {
        ...
        if (entry_uid == uid) {
            ...
            memcpy(out_buf, (const char*)name_table + byte_offset, name_len);
            out_buf[name_len] = '\0';
            return out_buf;
        }
