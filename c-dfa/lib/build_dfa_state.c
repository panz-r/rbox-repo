...
build_dfa_state_t* build_dfa_state_create(int alphabet_size, int initial_nfa_capacity) {
    ...
    state->transitions = malloc(sizeof(int) * alphabet_size);
    state->transitions_from_any = malloc(sizeof(bool) * alphabet_size);
    state->marker_offsets = malloc(sizeof(uint32_t) * alphabet_size);
    state->nfa_states = malloc(sizeof(int) * initial_nfa_capacity);
...
bool build_dfa_state_grow_nfa(build_dfa_state_t* state, int additional) {
    ...
    int* new_states = realloc(state->nfa_states, sizeof(int) * new_capacity);
    if (!new_states) return false;

    state->nfa_states = new_states;
