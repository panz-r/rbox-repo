...
static void test_binary_ops_and_template_interactions(void)
{
    ...
    soft_access_ctx_t ctx = {SOFT_OP_READ, "/src/data.txt", NULL, NULL};
    ...
    ctx = (soft_access_ctx_t){SOFT_OP_WRITE, "/dst/output.txt", NULL, NULL};
    ...
    ctx = (soft_access_ctx_t){SOFT_OP_COPY, "/data/file.txt", "/data/out.txt", NULL};
    result = soft_ruleset_check_ctx(rs, &ctx, &__g, NULL);
    ...
    result = soft_ruleset_check_ctx(rs, &ctx, &__g, NULL);
    TEST_ASSERT((result) && __g == 0, "template_resolve: SRC matches but DST missing, COPY denied");
    soft_ruleset_free(rs);
}
...
static void test_binary_serialization_all_variants(void)
{
    ...
    struct { const char *path; soft_binary_op_t op; const char *subject; } queries[] = {
        ...
        {"/data/project/file.txt", SOFT_OP_READ, "/usr/bin/user"},
    };
    ...
    ctx = (soft_access_ctx_t){SOFT_OP_READ, "/etc/hosts", NULL, NULL};
    TEST_ASSERT(soft_ruleset_check_ctx(rs2, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "bin_only_static: first rule works");

    ctx.src_path = "/etc/passwd";
    TEST_ASSERT(soft_ruleset_check_ctx(rs2, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "bin_only_static: second rule works");

    ctx = (soft_access_ctx_t){SOFT_OP_EXEC, "/usr/bin/gcc", NULL, NULL};
    TEST_ASSERT(soft_ruleset_check_ctx(rs2, &ctx, &__g, NULL) && __g == SOFT_ACCESS_EXEC, "bin_only_static: third rule works");

    /* Unmatched path should be undetermined */
    ctx = (soft_access_ctx_t){SOFT_OP_READ, "/etc/shadow", NULL, NULL};
    TEST_ASSERT(!soft_ruleset_check_ctx(rs2, &ctx, &__g, NULL) && __g == 0, "bin_only_static: unmatched path undetermined");
    ...
    struct { const char *path; soft_binary_op_t op; const char *subject; } tests[] = {
        {"/etc/hosts", SOFT_OP_READ, NULL},
        {"/data/file.txt", SOFT_OP_READ, NULL},
        {"/admin/config", SOFT_OP_READ, "/usr/bin/admin"},
        {"/admin/config", SOFT_OP_READ, "/usr/bin/user"},
        {"/restricted/file", SOFT_OP_READ, NULL},
        {"/restricted/file", SOFT_OP_READ, NULL},
        {"/nowhere", SOFT_OP_READ, NULL},
    };
...
static void test_batch_evaluation_edge_cases(void)
{
    ...
    ctx_arr[0].op = SOFT_OP_READ; ctx_arr[0].src_path = "/data/secret";
    ctx_arr[1].op = SOFT_OP_READ; ctx_arr[1].src_path = "/other/file.txt";
    ctx_arr[2].op = SOFT_OP_READ; ctx_arr[2].src_path = "/data/secret";
    const soft_access_ctx_t *c[3];
    c[0] = &ctx_arr[0]; c[1] = &ctx_arr[1]; c[2] = &ctx_arr[2];

    TEST_ASSERT_EQ(soft_ruleset_check_batch_ctx(rs, c, __g_arr, 3), 0,
                   "batch_all_denied: batch succeeds");
    TEST_ASSERT_EQ(__g_arr[0], 0, "batch_all_denied: first path denied");
    TEST_ASSERT_EQ(__g_arr[1], 0, "batch_all_denied: second path denied");
    TEST_ASSERT_EQ(__g_arr[2], 0, "batch_all_denied: third path denied");
    ...
    soft_access_ctx_t ctx = {SOFT_OP_READ, "/data/file1.txt", NULL, NULL};
    soft_ruleset_check_ctx(rs, &ctx, &__g, NULL);

    /* Batch: first entry cached, others not */
    memset(ctx_arr, 0, sizeof(ctx_arr));
    ctx_arr[0].op = SOFT_OP_READ; ctx_arr[0].src_path = "/data/file1.txt";
    ctx_arr[1].op = SOFT_OP_READ; ctx_arr[1].src_path = "/data/file2.txt";
    ctx_arr[2].op = SOFT_OP_READ; ctx_arr[2].src_path = "/data/file3.txt";
    c[0] = &ctx_arr[0]; c[1] = &ctx_arr[1]; c[2] = &ctx_arr[2];

    TEST_ASSERT_EQ(soft_ruleset_check_batch_ctx(rs, c, __g_arr, 3), 0,
                   "batch_mixed: batch succeeds");
    TEST_ASSERT_EQ(__g_arr[0], SOFT_ACCESS_READ, "batch_mixed: cached path allowed");
    TEST_ASSERT_EQ(__g_arr[1], SOFT_ACCESS_READ, "batch_mixed: uncached path1 allowed");
    TEST_ASSERT_EQ(__g_arr[2], SOFT_ACCESS_READ, "batch_mixed: uncached path2 allowed");

    /* Batch with NULL entry in middle */
    c[1] = NULL;
    TEST_ASSERT_EQ(soft_ruleset_check_batch_ctx(rs, c, __g_arr, 3), 0,
                   "batch_null_entry: batch with NULL entry succeeds");
    TEST_ASSERT_EQ(__g_arr[0], SOFT_ACCESS_READ, "batch_null_entry: first result correct");
    TEST_ASSERT_EQ(__g_arr[1], 0, "batch_null_entry: NULL entry returns 0");
    TEST_ASSERT_EQ(__g_arr[2], SOFT_ACCESS_READ, "batch_null_entry: third result correct");

    soft_ruleset_free(rs);

    /* Batch with different subjects */
    rs = soft_ruleset_new();
    soft_ruleset_add_rule_at_layer(rs, 0, "/data/**", SOFT_ACCESS_READ,
                                   SOFT_OP_READ, NULL, "**admin", SOFT_RULE_RECURSIVE);
    soft_ruleset_compile(rs);
    ...
    ctx_arr[0].subject = "/usr/bin/admin";
    ctx_arr[1].op = SOFT_OP_READ; ctx_arr[1].src_path = "/data/file.txt";
    ctx_arr[1].subject = "/usr/bin/admin";
    ctx_arr[2].op = SOFT_OP_READ; ctx_arr[2].src_path = "/data/file.txt";
    ctx_arr[2].subject = "/usr/bin/user";
    c[0] = &ctx_arr[0]; c[1] = &ctx_arr[1]; c[2] = &ctx_arr[2];

    TEST_ASSERT_EQ(soft_ruleset_check_batch_ctx(rs, c, __g_arr, 3), 0,
                   "batch_subjects: batch succeeds");
    TEST_ASSERT_EQ(__g_arr[0], SOFT_ACCESS_READ, "batch_subjects: matching subject allowed");
    TEST_ASSERT_EQ(__g_arr[1], SOFT_ACCESS_READ, "batch_subjects: matching subject allowed (duplicate)");
    TEST_ASSERT_EQ(__g_arr[2], 0, "batch_subjects: non-matching subject denied");
...
static void test_query_cache_binary_operations(void)
{
    ...
    soft_access_ctx_t ctx = {SOFT_OP_MOVE, "/src/file.txt", "/dst/out.txt", NULL};
    int result1 = soft_ruleset_check_ctx(rs, &ctx, &__g, NULL);
    TEST_ASSERT((result1) && __g == 0, "cache_bin_both: first MOVE result");

    /* Second MOVE with same paths - should hit cache for both */
    int result2 = soft_ruleset_check_ctx(rs, &ctx, &__g, NULL);
    TEST_ASSERT((result2) && __g == 0, "cache_bin_both: cached MOVE returns same");

    /* Third call should also return same cached result */
    int result3 = soft_ruleset_check_ctx(rs, &ctx, &__g, NULL);
    TEST_ASSERT((result3) && __g == 0, "cache_bin_both: third cached MOVE returns same");
