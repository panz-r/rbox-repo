...
static void test_deny_short_circuit_behavior(void)
{
    ...
    soft_access_ctx_t ctx = {SOFT_OP_READ, "/data/secret", NULL, NULL};
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == 0, "spec_static_deny: SPECIFICITY static DENY overrides PRECEDENCE");

    ctx.src_path = "/data/other/file.txt";
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "spec_static_deny: non-secret allowed by PRECEDENCE");
    ...
    ctx = (soft_access_ctx_t){SOFT_OP_READ, "/etc/shadow", NULL, NULL};
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == 0, "static_deny_exact: exact DENY short-circuits intersection");
    ...
    ctx = (soft_access_ctx_t){SOFT_OP_READ, "/data/anything/file.txt", NULL, NULL};
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == 0, "prec_dynamic_deny: PRECEDENCE dynamic DENY short-circuits");

    ctx.src_path = "/data/project/file.txt";
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == 0, "prec_dynamic_deny: DENY applies even to nested paths");
    ...
    ctx = (soft_access_ctx_t){SOFT_OP_READ, "/data/secret/passwords", NULL, NULL};
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == 0, "spec_dynamic_deny: SPEC dynamic DENY overrides PRECEDENCE");

    ctx.src_path = "/data/public/file.txt";
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "spec_dynamic_deny: non-secret allowed by PRECEDENCE");
...
static void test_four_buckets_integration(void)
{
    ...
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_EXEC, "four_buckets: SPEC static EXEC match");

    /* Test 2: SPECIFICITY dynamic match -> returns RWX */
    ctx = (soft_access_ctx_t){SOFT_OP_READ, "/data/repo/file.txt", NULL, NULL};
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && (__g == (SOFT_ACCESS_READ | SOFT_ACCESS_WRITE | SOFT_ACCESS_EXEC)), "four_buckets: SPEC dynamic RWX match");

    /* Test 3: Neither SPEC matches -> PRECEDENCE intersection */
    ctx = (soft_access_ctx_t){SOFT_OP_READ, "/data/static", NULL, NULL};
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "four_buckets: PREC intersection READ");

    /* Test 4: Only PREC dynamic matches */
    ctx.src_path = "/data/other/file.txt";
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && (__g == (SOFT_ACCESS_READ | SOFT_ACCESS_WRITE)), "four_buckets: only PREC dynamic RW");
...
static void test_layer_mask_compiled_evaluation(void)
{
    ...
    soft_access_ctx_t ctx = {SOFT_OP_EXEC, "/usr/bin/gcc", NULL, NULL};
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_EXEC, "mask_op_skip: EXEC query matches EXEC-only layer");
    ...
    ctx = (soft_access_ctx_t){SOFT_OP_READ, "/data/file.txt", NULL, NULL};
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "layer_masks: READ query uses READ-masked layer");

    /* EXEC query: only layer 0 evaluated */
    ctx = (soft_access_ctx_t){SOFT_OP_EXEC, "/bin/program", NULL, NULL};
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_EXEC, "layer_masks: EXEC query uses EXEC-masked layer");

    /* WRITE query denied (no layer has WRITE mask) */
    ctx = (soft_access_ctx_t){SOFT_OP_WRITE, "/data/file.txt", NULL, NULL};
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == 0, "layer_masks: WRITE query denied (no WRITE mask)");
...
static void test_deny_allow_and_mode_intersection(void)
{
    ...
    soft_access_ctx_t ctx = {SOFT_OP_READ, "/data/secret", NULL, NULL};
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == 0, "static_deny_dynamic_allow: DENY wins over ALLOW");

    ctx.src_path = "/data/public/file.txt";
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "static_deny_dynamic_allow: non-secret allowed");
    ...
    ctx = (soft_access_ctx_t){SOFT_OP_READ, "/data/file.txt", NULL, NULL};
    int result = soft_ruleset_check_ctx(rs, &ctx, &__g, NULL);
    TEST_ASSERT((result) && __g == 0, "same_pattern: disjoint modes result in denial");
    ...
    ctx = (soft_access_ctx_t){SOFT_OP_READ, "/data/file.txt", NULL, NULL};
    result = soft_ruleset_check_ctx(rs, &ctx, &__g, NULL);
    TEST_ASSERT((result) && __g == SOFT_ACCESS_READ, "same_pattern: overlapping modes result in intersection");
    ...
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == 0, "multi_deny: static DENY blocks /data/secret");

    /* Dynamic DENY blocks */
    ctx.src_path = "/data/forbidden/file.txt";
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == 0, "multi_deny: dynamic DENY blocks /data/forbidden/**");

    /* ALLOW grants access */
    ctx.src_path = "/data/public/file.txt";
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "multi_deny: ALLOW grants /data/public/**");

    /* Deep path under forbidden also denied */
    ctx.src_path = "/data/forbidden/deep/nested/file.txt";
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == 0, "multi_deny: deep forbidden path also denied");
    ...
    ctx = (soft_access_ctx_t){SOFT_OP_READ, "/data/public/file.txt", NULL, NULL};
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "multi_precedence_deny: public path allowed");

    /* /data/secret: layer 1 DENY blocks */
    ctx.src_path = "/data/secret/file.txt";
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == 0, "multi_precedence_deny: secret path denied by layer 1");

    /* /data/private: layer 2 DENY blocks */
    ctx.src_path = "/data/private/file.txt";
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == 0, "multi_precedence_deny: private path denied by layer 2");

    /* Deep secret path also denied */
    ctx.src_path = "/data/secret/deep/nested/file.txt";
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == 0, "multi_precedence_deny: deep secret path also denied");
...
static void test_specificity_dynamic_with_subject(void)
{
    ...
    /* Matching subject */
    soft_access_ctx_t ctx = {SOFT_OP_READ, "/admin/config", NULL, "/usr/bin/sudo"};
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "spec_dynamic_subject: matching subject allowed");

    /* Non-matching subject */
    ctx.subject = "/usr/bin/cat";
    TEST_ASSERT(!soft_ruleset_check_ctx(rs, &ctx, &__g, NULL), "spec_dynamic_subject: non-matching subject denied");

    /* Empty subject also denied */
    ctx.subject = "";
    TEST_ASSERT(!soft_ruleset_check_ctx(rs, &ctx, &__g, NULL), "spec_dynamic_subject: empty subject denied");

    soft_ruleset_free(rs);
}

/* ------------------------------------------------------------------ */
/*  Subject constraint tests in compiled ruleset                        */
/* ------------------------------------------------------------------ */

static void test_subject_constraints_compiled(void)
{
    uint32_t __g = 0;
    /* Rule with subject regex constraint */
    soft_ruleset_t *rs = soft_ruleset_new();
    soft_ruleset_add_rule_at_layer(rs, 0, "/data/...", SOFT_ACCESS_READ,
                                   SOFT_OP_READ, NULL, "**sudo", SOFT_RULE_RECURSIVE);
    soft_ruleset_compile(rs);
    ...
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "subject: constraint satisfied");

    /* Test with different subject */
    ctx.subject = "/usr/bin/cat";
    TEST_ASSERT(!soft_ruleset_check_ctx(rs, &ctx, &__g, NULL), "combined: different subject denied");
...
static void test_layer_type_transitions(void)
{
    ...
    soft_access_ctx_t ctx = {SOFT_OP_READ, "/data/secret/file.txt", NULL, NULL};
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == 0, "layer_transitions: SPECIFICITY DENY overrides");
    ...
    ctx.src_path = "/data/project/file.txt";
    int result = soft_ruleset_check_ctx(rs, &ctx, &__g, NULL);
    TEST_ASSERT((result) && __g == 0, "layer_transitions: PRECEDENCE intersection of disjoint modes denied");
...
void test_compilation_layered_run(void)
{
    ...
    RUN_TEST(test_subject_constraints_compiled);
