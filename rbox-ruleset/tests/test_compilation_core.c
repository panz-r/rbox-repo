...
static void test_subject_constraints_and_redunancy(void)
{
    ...
    ctx = (soft_access_ctx_t){SOFT_OP_READ, "/data/file.txt", NULL, "admin"};
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "subject_suffix: bare 'admin' matches");

    /* Subject ending with "sudo" doesn't match */
    ctx.subject = "/usr/bin/sudo";
    TEST_ASSERT(!soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == 0, "subject_suffix: 'sudo' doesn't match 'admin'");
    ...
    rs = soft_ruleset_new();
    soft_ruleset_add_rule_at_layer(rs, 0, "/data/**", SOFT_ACCESS_READ,
                                   SOFT_OP_READ, NULL, "/usr/bin/admin", SOFT_RULE_RECURSIVE);
    soft_ruleset_compile(rs);
    ...
    /* Partial prefix undetermined */
    ctx.subject = "/usr/bin";
    TEST_ASSERT(!soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == 0, "subject_exact: partial prefix undetermined");
...
static void test_zero_mode_template_prefix_and_spec(void)
{
    ...
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == 0, "zero_mode: disjoint modes result in denial");
    ...
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == 0, "template: COPY denied (no DST rule)");
    ...
    ctx = (soft_access_ctx_t){SOFT_OP_READ, "/data/project/sub", NULL, NULL};
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "prefix_match: matches /data/project prefix");

    /* /data/file matches /data prefix */
    ctx.src_path = "/data/file.txt";
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "prefix_match: matches /data prefix");
    ...
    ctx = (soft_access_ctx_t){SOFT_OP_READ, "/data/project/file.txt", NULL, NULL};
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "spec_longest: single pattern wins");
    ...
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == 0, "spec_deny: SPECIFICITY DENY overrides PRECEDENCE");

    ctx.src_path = "/data/public/file.txt";
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "spec_deny: non-secret allowed by PRECEDENCE");
    ...
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "intersect: static READ & dynamic READ|WRITE = READ");

    /* /data/other.txt: only dynamic matches -> READ|WRITE */
    ctx.src_path = "/data/other.txt";
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && (__g == (SOFT_ACCESS_READ | SOFT_ACCESS_WRITE)), "intersect: only dynamic matches -> READ|WRITE");
...
static void test_binary_search_and_exact_boundaries(void)
{
    ...
    soft_ruleset_add_rule(rs, "/zzz", SOFT_ACCESS_READ,
                          SOFT_OP_READ, NULL, NULL, 0);
    soft_ruleset_compile(rs);
    soft_access_ctx_t ctx = {SOFT_OP_READ, "/aaa", NULL, NULL};
    TEST_ASSERT(!soft_ruleset_check_ctx(rs, &ctx, &__g, NULL), "binsearch_before: no match when query before all rules");
    soft_ruleset_free(rs);
    ...
    soft_ruleset_compile(rs);
    ctx = (soft_access_ctx_t){SOFT_OP_READ, "/data/file.txt", NULL, NULL};
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "exact_length: exact match at same length");
    ctx.src_path = "/data/file.txt/";
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "exact_length: longer path matches as prefix");
    ctx.src_path = "/data/file";
    TEST_ASSERT(!soft_ruleset_check_ctx(rs, &ctx, &__g, NULL), "exact_length: shorter path doesn't match");
    soft_ruleset_free(rs);
}
...
static void test_pattern_boundary_and_universal_matching(void)
{
    ...
    soft_access_ctx_t ctx = {SOFT_OP_READ, pattern, NULL, NULL};
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "max_len_exact: exact max-length pattern matches");
    ...
    TEST_ASSERT(!soft_ruleset_check_ctx(rs, &ctx, &__g, NULL), "max_len_exact: different last char doesn't match");
    ...
    TEST_ASSERT(!soft_ruleset_check_ctx(rs, &ctx, &__g, NULL), "max_len_exact: shorter prefix doesn't match");
    ...
    for (i = 0; i < sizeof(paths)/sizeof(paths[0]); i++) {
        ctx = (soft_access_ctx_t){SOFT_OP_READ, paths[i], NULL, NULL};
        TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "ds_universal: matches every path");
    }
