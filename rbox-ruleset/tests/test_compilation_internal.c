...
static void test_eval_effective_path_full_interaction(void)
{
    ...
    soft_access_ctx_t ctx = {SOFT_OP_EXEC, "/data/project", NULL, NULL};
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_EXEC, "eval_full: SPEC static EXEC match");

    /* Test 2: SPECIFICITY dynamic match -> returns READ (for READ query) */
    ctx = (soft_access_ctx_t){SOFT_OP_READ, "/data/repo/file.txt", NULL, NULL};
    ...
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "eval_full: PREC intersection READ");

    /* Test 4: Only PREC dynamic matches */
    ctx.src_path = "/data/other/file.txt";
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && (__g == (SOFT_ACCESS_READ | SOFT_ACCESS_WRITE)), "eval_full: only PREC dynamic RW");
...
static void test_static_dynamic_matching_and_intersection(void)
{
    ...
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "static_binsearch: first exact match");

    /* Query exact match for middle rule */
    ctx.src_path = "/data/dir15";
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "static_binsearch: middle exact match");

    /* Query exact match for last rule */
    ctx.src_path = "/data/dir29";
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "static_binsearch: last exact match");

    /* Query prefix match (should scan backward from insertion point) */
    ctx.src_path = "/data/dir15/subdir/file.txt";
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "static_binsearch: middle prefix match");

    /* Query non-existent path */
    ctx.src_path = "/data/dir99";
    TEST_ASSERT(!soft_ruleset_check_ctx(rs, &ctx, &__g, NULL), "static_binsearch: non-existent denied");

    /* Query before all rules */
    ctx.src_path = "/data/dir000";
    TEST_ASSERT(!soft_ruleset_check_ctx(rs, &ctx, &__g, NULL), "static_binsearch: before all denied");
    ...
    ctx = (soft_access_ctx_t){SOFT_OP_READ, "/data/file.txt", NULL, NULL};
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == 0, "dynamic_intersection: disjoint modes denied");
    ...
    ctx = (soft_access_ctx_t){SOFT_OP_READ, "/data/file.txt", NULL, NULL};
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "dynamic_intersection: overlapping modes give READ");
    ...
    ctx = (soft_access_ctx_t){SOFT_OP_READ, "/data/file.txt", NULL, NULL};
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == 0, "dynamic_deny: DENY short-circuits");
...
static void test_specificity_matching_and_deny(void)
{
    ...
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "spec_static: exact match");

    /* Non-match */
    ctx.src_path = "/data/other.txt";
    TEST_ASSERT(!soft_ruleset_check_ctx(rs, &ctx, &__g, NULL), "spec_static: non-match denied");
    ...
    /* Recursive match */
    ctx = (soft_access_ctx_t){SOFT_OP_READ, "/data/deep/file.txt", NULL, NULL};
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "spec_dynamic: recursive match");
    ...
    ctx = (soft_access_ctx_t){SOFT_OP_READ, "/data/subdir/file.txt", NULL, NULL};
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "spec_longest: longer pattern wins");

    /* /data/other.txt matches shorter pattern -> READ */
    ctx.src_path = "/data/other.txt";
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "spec_longest: shorter pattern wins");
    ...
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == 0, "deny_static: static DENY short-circuits");
    ...
    ctx = (soft_access_ctx_t){SOFT_OP_READ, "/data/secret", NULL, NULL};
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == 0, "deny_spec_static: SPEC static DENY");
    ...
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == 0, "deny_spec_dynamic: SPEC dynamic DENY");
...
static void test_complex_combinations_and_edge_cases(void)
{
    ...
    soft_access_ctx_t ctx = {SOFT_OP_READ, "/data/file.txt", NULL, NULL};
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "complex_combo: exact + dynamic intersection");

    /* /data/subdir matches prefix + dynamic -> EXEC & (READ|WRITE) = 0 -> denied */
    ctx.src_path = "/data/subdir";
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == 0, "complex_combo: prefix + dynamic disjoint intersection");

    /* /data/other.txt matches only dynamic -> READ|WRITE */
    ctx.src_path = "/data/other.txt";
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && (__g == (SOFT_ACCESS_READ | SOFT_ACCESS_WRITE)), "complex_combo: only dynamic matches");
    ...
    ctx = (soft_access_ctx_t){SOFT_OP_READ, "/data/secret", NULL, NULL};
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == 0, "complex_deny: static DENY short-circuits");

    /* /data/public: only dynamic matches -> READ */
    ctx.src_path = "/data/public/file.txt";
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "complex_deny: dynamic allows non-secret path");
    ...
    TEST_ASSERT(!soft_ruleset_check_ctx(rs, &ctx, &__g, NULL), "eval_edge_empty: empty path denied");
    ...
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "eval_edge_single: single char path matches");
    ...
    ctx = (soft_access_ctx_t){SOFT_OP_READ, "/data/file with spaces.txt", NULL, NULL};
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "eval_edge_spaces: path with spaces matches");
    ...
    ctx = (soft_access_ctx_t){SOFT_OP_READ, long_path, NULL, NULL};
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "eval_edge_long: very long path matches");
...
static void test_subject_subsumption_and_layer_configs(void)
{
    ...
    /* Exact match */
    soft_access_ctx_t ctx = {SOFT_OP_READ, "/data/file.txt", NULL, "/usr/bin/sudo"};
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "subject_exact: exact subject matches");

    /* NULL subject with regex present -> undetermined */
    ctx.subject = NULL;
    TEST_ASSERT(!soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == 0, "subject_exact: NULL subject undetermined when regex present");
    ...
    rs = soft_ruleset_new();
    soft_ruleset_add_rule_at_layer(rs, 0, "/data/**", SOFT_ACCESS_READ,
                                   SOFT_OP_READ, NULL, "**root", SOFT_RULE_RECURSIVE);
    soft_ruleset_compile(rs);

    /* Subject ending with "root" */
    ctx = (soft_access_ctx_t){SOFT_OP_READ, "/data/file.txt", NULL, "/usr/bin/root"};
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "subject_suffix: root matches **root$");

    /* Subject ending with "superroot" */
    ctx.subject = "/usr/bin/superroot";
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "subject_suffix: superroot matches **root$");

    /* Subject with "root" in middle but not at end */
    ctx.subject = "/usr/bin/rootkit";
    TEST_ASSERT(!soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == 0, "subject_suffix: rootkit doesn't match **root$");
    ...
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "mixed_patterns: deep path matches recursive");

    /* Exact path matches exact rule */
    ctx.src_path = "/etc/hosts";
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "mixed_patterns: exact path matches exact rule");

    /* Path under /etc but not exact -> denied */
    ctx.src_path = "/etc/other";
    TEST_ASSERT(!soft_ruleset_check_ctx(rs, &ctx, &__g, NULL), "mixed_patterns: /etc/other denied (no recursive)");
    ...
    ctx = (soft_access_ctx_t){SOFT_OP_READ, "/data/file.txt", NULL, NULL};
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "compile_single: single rule works");
    soft_ruleset_free(rs);
    ...
    ctx = (soft_access_ctx_t){SOFT_OP_READ, "/data/secret/file.txt", NULL, NULL};
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == 0, "compile_many_layers: SPECIFICITY deny overrides");

    /* /data/project: PRECEDENCE intersection (READ & WRITE = 0) */
    ctx.src_path = "/data/project/file.txt";
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == 0, "compile_many_layers: PRECEDENCE intersection denied");

    /* /data/other: only layer 0 matches -> READ */
    ctx.src_path = "/data/other/file.txt";
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "compile_many_layers: layer 0 allows");
