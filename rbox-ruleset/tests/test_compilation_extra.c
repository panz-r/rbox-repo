...
static void test_templates_and_binary_operations(void)
{
    ...
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == 0, "template_prefix: SRC matches but DST missing, denied");
...
static void test_subject_constraint_edge_cases(void)
{
    ...
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "subject_anchor: admin matches **admin$");

    /* Subject ending with "myadmin" also matches */
    ctx.subject = "/usr/bin/myadmin";
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "subject_anchor: myadmin matches **admin$");

    /* Subject with "admin" in middle doesn't match */
    ctx.subject = "/usr/bin/admin_extra";
    TEST_ASSERT(!soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == 0, "subject_anchor: admin_extra doesn't match **admin$");
    ...
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "subject_exact_match: exact subject matches");

    /* Similar but longer */
    ctx.subject = "/usr/bin/sudo/extra";
    TEST_ASSERT(!soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == 0, "subject_exact_match: longer subject undetermined");
    ...
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "subject_empty_vs_null: NULL subject allowed");

    /* Empty string subject also matches */
    ctx.subject = "";
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "subject_empty_vs_null: empty subject allowed");

    soft_ruleset_free(rs);
}

/* ------------------------------------------------------------------ */
/*  DENY rules and subject constraint edge cases                        */
/* ------------------------------------------------------------------ */

static void test_deny_rules_and_subject_edge_cases(void)
{
    uint32_t __g = 0;
    ...
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == 0, "multi_deny: secret denied");

    /* /data/private: second DENY blocks */
    ctx.src_path = "/data/private/file.txt";
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == 0, "multi_deny: private denied");

    /* /data/public: ALLOW works */
    ctx.src_path = "/data/public/file.txt";
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "multi_deny: public allowed");
    ...
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "subject_constraint: matching subject allowed");

    /* Non-matching subject denied */
    ctx.subject = "/usr/bin/user";
    TEST_ASSERT(!soft_ruleset_check_ctx(rs, &ctx, &__g, NULL), "subject_constraint: non-matching subject denied");
...
static void test_pattern_flags_and_minimal_cases(void)
{
    ...
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "same_pattern_flags: same modes intersect to READ");
    ...
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "minimal_pattern: single char exact match");
    ...
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "minimal_pattern: root path matches");

    /* Any path */
    ctx.src_path = "/any/deep/path";
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "minimal_pattern: any path matches");
...
static void test_query_cache_and_many_layers(void)
{
    ...
    TEST_ASSERT((result1) && __g == SOFT_ACCESS_READ, "cache_subject: admin allowed");

    /* Different subject - undetermined (different cache entry) */
    ctx.subject = "/usr/bin/user";
    int result2 = soft_ruleset_check_ctx(rs, &ctx, &__g, NULL);
    TEST_ASSERT(!(result2) && __g == 0, "cache_subject: user undetermined");

    /* Admin again - should hit cache */
    ctx.subject = "/usr/bin/admin";
    int result3 = soft_ruleset_check_ctx(rs, &ctx, &__g, NULL);
    TEST_ASSERT((result3) && __g == SOFT_ACCESS_READ, "cache_subject: admin still allowed (cache)");
    ...
    ctx = (soft_access_ctx_t){SOFT_OP_READ, "/data/file.txt", NULL, NULL};
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "many_layers: general allow works");

    /* /data/secret/file.txt: layer 1 SPECIFICITY DENY -> denied */
    ctx.src_path = "/data/secret/file.txt";
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == 0, "many_layers: secret denied");

    /* /data/public/file.txt: layer 3 SPECIFICITY allows -> READ */
    ctx.src_path = "/data/public/file.txt";
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == SOFT_ACCESS_READ, "many_layers: public allowed");

    /* /data/forbidden/file.txt: layer 4 PRECEDENCE DENY -> denied */
    ctx.src_path = "/data/forbidden/file.txt";
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, &__g, NULL) && __g == 0, "many_layers: forbidden denied");
...
void test_compilation_extra_run(void)
{
    ...
    RUN_TEST(test_deny_rules_and_subject_edge_cases);
