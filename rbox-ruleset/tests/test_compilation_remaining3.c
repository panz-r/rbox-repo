...
static void test_binary_serialization_subject_and_empty(void)
{
    ...
    if (len > 0) {
        ...
        if (rs2 != NULL) {
            ctx = (soft_access_ctx_t){SOFT_OP_READ, "/anything", NULL, NULL, 1000};
            TEST_ASSERT_EQ(soft_ruleset_check_ctx(rs2, &ctx, NULL), -13,
                           "empty_load: loaded empty ruleset denies all");
            soft_ruleset_free(rs2);
        }
    }
...
static void test_query_cache_and_subject_constraints(void)
{
    ...
    soft_access_ctx_t ctx = {SOFT_OP_COPY, "/src/file.txt", "/dst/out.txt", NULL, 1000};
    ...
    TEST_ASSERT_EQ(soft_ruleset_check_ctx(rs, &ctx, NULL), SOFT_ACCESS_READ,
                   "subject_empty: NULL subject allowed");

    /* Empty string subject also matches */
    ctx.subject = "";
    TEST_ASSERT_EQ(soft_ruleset_check_ctx(rs, &ctx, NULL), SOFT_ACCESS_READ,
                   "subject_empty: empty string subject allowed");

    /* Any non-NULL subject also matches */
    ctx.subject = "/usr/bin/anything";
    TEST_ASSERT_EQ(soft_ruleset_check_ctx(rs, &ctx, NULL), SOFT_ACCESS_READ,
                   "subject_empty: any subject allowed");
    ...
    soft_ruleset_add_rule_at_layer(rs, 0, "/data/**", SOFT_ACCESS_READ,
                                   SOFT_OP_READ, NULL, long_regex, 0, SOFT_RULE_RECURSIVE);
    soft_ruleset_compile(rs);
    ...
    ctx = (soft_access_ctx_t){SOFT_OP_READ, "/data/file.txt", NULL, match_subject, 1000};
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, NULL) != -13 ||
                soft_ruleset_check_ctx(rs, &ctx, NULL) == -13,
...
static void test_invalidation_recompile_and_shadow(void)
{
    ...
    soft_access_ctx_t ctx = {SOFT_OP_READ, "/data/file0.txt", NULL, NULL, 1000};
    TEST_ASSERT_EQ(soft_ruleset_check_ctx(rs, &ctx, NULL), SOFT_ACCESS_READ,
                   "multi_recompile_1: static rule works");
    ...
    ctx = (soft_access_ctx_t){SOFT_OP_READ, "/data/file5.txt", NULL, NULL, 1000};
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, NULL) != -13,
    ...
    ctx = (soft_access_ctx_t){SOFT_OP_READ, "/data/important/secret.txt", NULL, NULL, 1000};
    TEST_ASSERT_EQ(soft_ruleset_check_ctx(rs, &ctx, NULL), -13,
                   "multi_recompile_3: SPECIFICITY DENY works");

    /* Non-important still allowed */
    ctx.src_path = "/data/file0.txt";
    TEST_ASSERT_EQ(soft_ruleset_check_ctx(rs, &ctx, NULL), SOFT_ACCESS_READ,
                   "multi_recompile_3: non-important still allowed");
    ...
    ctx = (soft_access_ctx_t){SOFT_OP_READ, "/data/file.txt", NULL, NULL, 1000};
    TEST_ASSERT_EQ(soft_ruleset_check_ctx(rs, &ctx, NULL), SOFT_ACCESS_READ,
                   "shadow_elim: specific rule shadowed by general");
    ...
    ctx = (soft_access_ctx_t){SOFT_OP_READ, "/data/file.txt", NULL, NULL, 1000};
    TEST_ASSERT_EQ(soft_ruleset_check_ctx(rs, &ctx, NULL), SOFT_ACCESS_READ,
                   "shadow_no_elim: /data allowed");

    ctx.src_path = "/other/file.txt";
    TEST_ASSERT_EQ(soft_ruleset_check_ctx(rs, &ctx, NULL), SOFT_ACCESS_READ,
                   "shadow_no_elim: /other allowed");
    ...
    ctx = (soft_access_ctx_t){SOFT_OP_READ, "/data/file.txt", NULL, NULL, 1000};
    TEST_ASSERT_EQ(soft_ruleset_check_ctx(rs, &ctx, NULL), -13,
                   "deny_shadow: DENY shadows ALLOW");
    ...
    ctx = (soft_access_ctx_t){SOFT_OP_READ, "/data/file.txt", NULL, NULL, 1000};
    TEST_ASSERT_EQ(soft_ruleset_check_ctx(rs, &ctx, NULL), -13,
                   "zero_rules_shadow: shadowed rule results in DENY");

    ctx.src_path = "/data/other.txt";
    TEST_ASSERT_EQ(soft_ruleset_check_ctx(rs, &ctx, NULL), -13,
                   "zero_rules_shadow: general DENY blocks everything");
...
static void test_layer_mask_template_and_pattern_covers(void)
{
    ...
    TEST_ASSERT_EQ(soft_ruleset_check_ctx(rs, &ctx, NULL), SOFT_ACCESS_READ,
                   "mask_match: READ query allowed (mask=READ)");
    ...
    TEST_ASSERT_EQ(soft_ruleset_check_ctx(rs, &ctx, NULL), -13,
                   "template_dynamic: COPY denied (no DST rule)");
    ...
    ctx = (soft_access_ctx_t){SOFT_OP_READ, "/data/file.txt", NULL, NULL, 1000};
    TEST_ASSERT_EQ(soft_ruleset_check_ctx(rs, &ctx, NULL), SOFT_ACCESS_READ,
                   "covers_recursive: recursive shadows non-recursive");
    ...
    ctx = (soft_access_ctx_t){SOFT_OP_READ, "/data/file.txt", NULL, NULL, 1000};
    TEST_ASSERT_EQ(soft_ruleset_check_ctx(rs, &ctx, NULL), SOFT_ACCESS_READ,
                   "covers_no_recursive: non-recursive doesn't shadow recursive");

    ctx.src_path = "/data/other.txt";
    TEST_ASSERT_EQ(soft_ruleset_check_ctx(rs, &ctx, NULL), SOFT_ACCESS_READ,
                   "covers_no_recursive: recursive allows other paths");
