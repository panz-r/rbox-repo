...
static void test_template_variable_resolution(void)
{
    ...
    ctx = (soft_access_ctx_t){SOFT_OP_COPY, "/src/file.txt", "/data/out.txt", NULL, 1000};
    /* Template resolves ${DST} to "/data/out.txt", matches DST query path */
    /* But SRC has no rule, so COPY denied */
    TEST_ASSERT_EQ(soft_ruleset_check_ctx(rs, &ctx, NULL), -13,
                   "template_dst: DST matches but SRC missing, denied");
...
static void test_rule_constraints_equality(void)
{
    ...
    TEST_ASSERT_EQ(soft_ruleset_check_ctx(rs, &ctx, NULL), SOFT_ACCESS_READ,
                   "constraints_equal: identical rules shadow correctly");
    ...
    TEST_ASSERT_EQ(soft_ruleset_check_ctx(rs, &ctx, NULL), SOFT_ACCESS_READ,
                   "constraints_diff_op: different op_types survive");

    soft_ruleset_free(rs);

    /* Different min_uid -> not equal */
    rs = soft_ruleset_new();
    soft_ruleset_add_rule_at_layer(rs, 0, "/data/**", SOFT_ACCESS_READ,
                                   SOFT_OP_READ, NULL, NULL, 1000, SOFT_RULE_RECURSIVE);
    soft_ruleset_add_rule_at_layer(rs, 1, "/data/**", SOFT_ACCESS_READ,
                                   SOFT_OP_READ, NULL, NULL, 500, SOFT_RULE_RECURSIVE);
    soft_ruleset_compile(rs);

    /* Different UIDs -> both rules may survive depending on shadow logic */
    ctx = (soft_access_ctx_t){SOFT_OP_READ, "/data/file.txt", NULL, NULL, 1000};
    TEST_ASSERT(soft_ruleset_check_ctx(rs, &ctx, NULL) != -13,
                "constraints_diff_uid: different UIDs evaluated");
    ...
    TEST_ASSERT_EQ(soft_ruleset_check_ctx(rs, &ctx, NULL), SOFT_ACCESS_READ,
                   "constraints_diff_subject: different subjects survive");
...
static void test_subject_redundancy_elimination(void)
{
    ...
    TEST_ASSERT_EQ(soft_ruleset_check_ctx(rs, &ctx, NULL), SOFT_ACCESS_READ,
                   "subject_not_redundant: admin gets READ from intersection");
...
static void test_eff_add_capacity_growth(void)
{
    ...
    for (i = 0; i < 100; i++) {
        ...
        soft_ruleset_add_rule(rs, path, SOFT_ACCESS_READ, SOFT_OP_READ, NULL, NULL, 0, 0);
    }
    soft_ruleset_compile(rs);

    /* Verify all rules work */
    soft_access_ctx_t ctx = {SOFT_OP_READ, "/data/static_000", NULL, NULL, 1000};
    TEST_ASSERT_EQ(soft_ruleset_check_ctx(rs, &ctx, NULL), SOFT_ACCESS_READ,
                   "eff_add_static: first static rule works");

    ctx.src_path = "/data/static_099";
    TEST_ASSERT_EQ(soft_ruleset_check_ctx(rs, &ctx, NULL), SOFT_ACCESS_READ,
                   "eff_add_static: last static rule works");
    ...
    for (i = 0; i < 100; i++) {
        ...
        soft_ruleset_add_rule(rs, path, SOFT_ACCESS_READ, SOFT_OP_READ, NULL, NULL, 0, SOFT_RULE_RECURSIVE);
    }
    soft_ruleset_compile(rs);

    /* Verify dynamic rules work */
    ctx = (soft_access_ctx_t){SOFT_OP_READ, "/data/dyn_000/file.txt", NULL, NULL, 1000};
    TEST_ASSERT_EQ(soft_ruleset_check_ctx(rs, &ctx, NULL), SOFT_ACCESS_READ,
                   "eff_add_dynamic: first dynamic rule works");

    ctx.src_path = "/data/dyn_099/file.txt";
    TEST_ASSERT_EQ(soft_ruleset_check_ctx(rs, &ctx, NULL), SOFT_ACCESS_READ,
                   "eff_add_dynamic: last dynamic rule works");
...
static void test_query_cache_hit_miss_patterns(void)
{
    ...
    TEST_ASSERT_EQ(result1, SOFT_ACCESS_READ, "cache_subject_hit_miss: admin allowed");
    ...
    TEST_ASSERT_EQ(result3, SOFT_ACCESS_READ, "cache_subject_hit_miss: admin still allowed (cache hit)");

    soft_ruleset_free(rs);

    /* Same path, different UIDs -> different cache entries */
    rs = soft_ruleset_new();
    soft_ruleset_add_rule_at_layer(rs, 0, "/data/**", SOFT_ACCESS_READ,
                                   SOFT_OP_READ, NULL, NULL, 1000, SOFT_RULE_RECURSIVE);
    soft_ruleset_compile(rs);
    ...
    TEST_ASSERT_EQ(result1, SOFT_ACCESS_READ, "cache_uid_hit_miss: UID 1000 allowed");
    ...
    ctx.uid = 1000;
    result3 = soft_ruleset_check_ctx(rs, &ctx, NULL);
    TEST_ASSERT_EQ(result3, SOFT_ACCESS_READ, "cache_uid_hit_miss: UID 1000 still allowed (cache hit)");
    ...
    /* Path 1 */
    ctx = (soft_access_ctx_t){SOFT_OP_READ, "/data/file1.txt", NULL, NULL, 1000};
    result1 = soft_ruleset_check_ctx(rs, &ctx, NULL);
    TEST_ASSERT_EQ(result1, SOFT_ACCESS_READ, "cache_path_hit_miss: path1 allowed");

    /* Path 2 - different cache entry */
    ctx.src_path = "/data/file2.txt";
    result2 = soft_ruleset_check_ctx(rs, &ctx, NULL);
    TEST_ASSERT_EQ(result2, SOFT_ACCESS_READ, "cache_path_hit_miss: path2 allowed");

    /* Path 1 again - should hit cache */
    ctx.src_path = "/data/file1.txt";
    result3 = soft_ruleset_check_ctx(rs, &ctx, NULL);
    TEST_ASSERT_EQ(result3, SOFT_ACCESS_READ, "cache_path_hit_miss: path1 still allowed (cache hit)");
    ...
    ctx = (soft_access_ctx_t){SOFT_OP_READ, "/data/file.txt", NULL, NULL, 1000};
    TEST_ASSERT_EQ(soft_ruleset_check_ctx(rs, &ctx, NULL), SOFT_ACCESS_READ,
                   "cache_invalidate: first query allowed");
    ...
    TEST_ASSERT_EQ(soft_ruleset_check_ctx(rs, &ctx, NULL), -13,
                   "cache_invalidate: after add, query denied (cache invalidated)");
