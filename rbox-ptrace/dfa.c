...
extern const unsigned char rbox_ptrace_dfa_data[];
extern const size_t rbox_ptrace_dfa_data_size;
...
static void dfa_init_once_wrapper(void) {
    if (!dfa_eval_validate_id(rbox_ptrace_dfa_data, rbox_ptrace_dfa_data_size, EXPECTED_IDENTIFIER)) {
...
int dfa_should_allow(const char* cmd) {
    ...
    dfa_result_t result;
    if (dfa_eval(rbox_ptrace_dfa_data, rbox_ptrace_dfa_data_size, cmd, strlen(cmd), &result)) {
